...
if (isset($_POST['simpan'])) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($extFoto, $ext)) {
            ...
            $insert = mysqli_query($koneksi, "INSERT INTO project (nama_project, keterangan, foto) VALUES
            ('$nama_project', '$keterangan', '$nama_foto')");
        }
    } else {
        $insert = mysqli_query($koneksi, "INSERT INTO project (nama_project, keterangan) VALUES
            ('$nama_project', '$keterangan')");
    }
...
$editProject = mysqli_query(
    $koneksi,
    "SELECT * FROM project WHERE id = '$id'"
);
$rowEdit = mysqli_fetch_assoc($editProject);
...
if (isset($_POST['edit'])) {
    ...
    $update = mysqli_query($koneksi, "UPDATE project SET nama_project='$nama_project', keterangan='$keterangan', foto='$nama_foto' WHERE id='$id'");
                            ...
                            <div class="col-sm-12">
                                <div class="card shadow p-3 mb-5 bg-transparent rounded mt-5" style="backdrop-filter: blur(20px);>
                                    ...
                                    <legend class="float-none w-auto px-3 fw-bold">
                                        <?php echo isset($_GET['edit']) ? 'Edit' : 'Add' ?> Project</legend>
                                    <form action="" method="post" enctype="multipart/form-data">
                                        <div class="mb-3 row">
                                            <div class="col-sm-6">
                                                <label for="" class="form-label text-white">Nama</label>
                                                <input type="text" class="form-control bg-transparent text-white"
                                                    name="nama_project" placeholder="Masukkan Nama Project Anda"
                                                    required
                                                    value="<?php echo isset($_GET['edit']) ? $rowEdit['nama_project'] : '' ?>">
                                            </div>
                                            <div class="col-sm-12">
                                            ...
                                            <div class="mb-3 row">
                                                <div class="col-sm-3">
                                                    ...
                                                    <img width="150"
                                                        src="upload/<?php echo isset($rowProject['foto']) ? $rowEdit['foto'] : '' ?>"
                                                        alt="">
                                                </div>
                                            </div>
                                        </div>
