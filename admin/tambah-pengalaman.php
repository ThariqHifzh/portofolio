...
if (isset($_POST['simpan'])) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($extFoto, $ext)) {
            ...
            $insert = mysqli_query($koneksi, "INSERT INTO pengalaman (perusahaan, tahun, keterangan, foto) VALUES
            ('$pengalaman', '$tahun', '$keterangan', '$nama_foto')");
        }
    } else {
        $insert = mysqli_query($koneksi, "INSERT INTO pengalaman (perusahaan, tahun, keterangan) VALUES
...
$editUser = mysqli_query(
    $koneksi,
    "SELECT * FROM pengalaman WHERE id = '$id'"
);
...
if (isset($_POST['edit'])) {
    ...
    $update = mysqli_query($koneksi, "UPDATE pengalaman SET perusahaan='$pengalaman',tahun='$tahun', keterangan='$keterangan' WHERE id='$id'");
...
if (isset($_GET['delete'])) {
    ...
    header("location:pengalaman.php?hapus=berhasil");
}
?>
                            ...
                            <div class="col-sm-12">
                                <div class="card shadow p-3 mb-5 bg-transparent rounded mt-5" style="backdrop-filter: blur(20px);>
                                    ...
                                    <legend class="float-none w-auto px-3 fw-bold">
                                        <?php echo isset($_GET['edit']) ? 'Edit' : 'Add' ?> Pengalaman</legend>
                                    <form action="" method="post" enctype="multipart/form-data">
                                        <div class="mb-3 row">
                                            <div class="col-sm-6">
                                                <label for="" class="form-label text-white">Nama Perusahaan</label>
                                                <input type="text" class="form-control bg-transparent text-white"
                                                    name="perusahaan" placeholder="Masukkan Perusahaan" required
                                                    value="<?php echo isset($_GET['edit']) ? $rowEdit['perusahaan'] : '' ?>">
                                            </div>
                                            <div class="col-sm-6">
