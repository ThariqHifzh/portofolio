...
if (isset($_POST['simpan'])) {
    ...
    if(!empty($_FILES['foto']['name'])){
        ...
        if(!in_array($extFoto, $ext)){
            ...
            $insert = mysqli_query($koneksi, "INSERT INTO user (nama, email, password , foto) VALUES
            ('$nama', '$email', '$password', '$nama_foto')");
        }
    } else {
        $insert = mysqli_query($koneksi, "INSERT INTO user (nama, email, password ) VALUES
            ('$nama', '$email', '$password')");
    }
                                ...
                                <div class="card shadow p-3 mb-5 bg-transparent rounded mt-5" style="backdrop-filter: blur(20px);>
                                    <div class="card-body">
                                        <legend class="float-none w-auto px-3 fw-bold">
                                            <?php echo isset($_GET['edit']) ? 'Edit' : 'Add' ?> Profile</legend>
                                            <form action="" method="post" enctype="multipart/form-data">
                                                <div class="mb-3 row">
                                                    <div class="col-sm-6">
                                                        <label for="" class="form-label text-white">Nama</label>
                                                        <input type="text" class="form-control bg-transparent text-white" name="nama" placeholder="Masukkan Nama Anda" required value="<?php echo isset($_GET['edit']) ? $rowEdit['nama'] : '' ?>">
                                                    </div>
                                                    <div class="col-sm-6">
                                                        <label for="" class="form-label text-white">Email</label>
                                                        <input type="email" class="form-control bg-transparent text-white" name="email" placeholder="Masukkan Email Anda" required value="<?php echo isset($_GET['edit']) ? $rowEdit['email'] : '' ?>">
                                                    </div>
                                                </div>
                                                <div class="mb-3 row">
                                                    <div class="col-sm-12">
                                                        <label for="" class="form-label text-white">Password</label>
                                                        <input type="password" class="form-control bg-transparent text-white" name="password" placeholder="Masukkan Password" id="">
                                                    </div>
                                                </div>
                                                <div class="mb-3 row">
                                                    <div class="col-sm-7">
                                                        <label for="" class="form-label text-white">Foto</label>
                                                        <input type="file" class="form-control bg-transparent text-white" name="foto" >
                                                    </div>
                                                </div>
                                                <div class="mb-3">
